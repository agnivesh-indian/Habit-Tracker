// statsEngine.js
// This file will contain the core statistical functions.

// Basic math functions (simplified for this example, a real app might use a library like mathjs)
...
export const calculateCompletionRate = (habit) => {
    ...
    if (habit.goalType === 'cumulative') {
        if (!habit.goalAmount || habit.goalAmount === 0) return 0;
        const totalValue = habit.logs.reduce((s, log) => s + (log.value || 0), 0);
...
export const analyzeCorrelations = (primaryHabit, allHabits) => {
    ...
    for (const otherHabit of otherHabits) {
        const commonDates = primaryHabit.logs
            .map(log => log.date)
            .filter(date => otherHabit.logs.some(otherLog => otherLog.date === date));
        ...
        const primarySuccessDays = new Set(commonDates.filter(date => {
            ...
            return primaryHabit.goalType === 'daily' 
                ? log.completed 
                : (log.value || 0) >= (primaryHabit.goalAmount || Infinity);
        }));
